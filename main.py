...
# --- MUSIC DOWNLOAD LOGIC (Updated for Nursery Rhymes) ---
...
async def process_download(update_or_query, query, status_msg, original_msg_id):
    ...
    try:
        ...
        print(f"Error: {e}")
        try: await status_msg.edit_text("***❌ Error: Song not found or restricted.***", parse_mode='Markdown')
        except: pass
    finally:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Command detection for /get and /lr
    query = None
...
async def main():
    ...
    app.add_handler(CommandHandler("get", handle_text))
    app.add_handler(CommandHandler("lr", handle_text))
    app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))
    
    print(f"{GREEN}Bot running 24/7 on Railway ----{RESET}")
...
async def main():
    ...
    app.add_handler(CommandHandler("help", help_cmd))
    app.add_handler(CommandHandler("broadcast", broadcast))
    app.add_handler(CommandHandler("get", handle_text)) # Added command trigger
    app.add_handler(CommandHandler("lr", handle_text))  # Added command trigger
    app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))
